...
function TestSkylinkV1Bitfield(t: any) {
  ...
  for (let i = 0; i < tests.length; i++) {
    const [bitfield, errSVB] = skylinkV1Bitfield(BigInt(tests[i].trial));
    if (errSVB !== null) {
...
function TestPaddedFileSize(t: any) {
  ...
  for (let i = 0; i < tests.length; i++) {
    const suggestion = getPaddedFileSize(tests[i].trial);
...
function TestEncryptFileSmall(t: any) {
  ...
  const [encData2, err] = encryptFileSmall(seed, inode, revision + 1n, metadata, fileData);
  if (err !== null) {
  ...
  const [seed2, errVSP2] = seedPhraseToSeed(spd);
  if (errVSP2 !== null) {
    t.fail(errVSP2);
    return;
  }

  const [encData3, errEFS] = encryptFileSmall(seed2, inode, revision, metadata, fileData);
  if (errEFS !== null) {
    t.fail("could not encrypt file for revision test");
    return;
  }
...
function TestEncryptDecryptSpeed(t: any) {
  // Get a seed.
  const [seedPhrase, errGSPD] = generateSeedPhraseDeterministic("test-for-speed");
  if (errGSPD !== null) {
    t.fail(errGSPD);
    return;
  }
  ...
  // Establish the other inputs.
  const inode = "testFileSpeed";
  const revision = BigInt(0);
  const metadata = {
    filename: "testSpeed.txt",
  };
  const fileData = new Uint8Array(20 * 1000 * 1000);
