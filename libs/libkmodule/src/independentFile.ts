...
function createIndependentFileSmall(
  ...
  return new Promise(async (resolve) => {
    ...
    // function.
    const [inode, errNI] = namespaceInode("IndependentFileSmall", userInode);
    if (errNI !== null) {
    ...
    const [keypair, dataKey, errTREK] = taggedRegistryEntryKeys(seed, inode, inode);
    if (errTREK !== null) {
    ...
    const [result, errRR] = await registryRead(keypair.publicKey, dataKey);
    if (errRR !== null) {
    ...
    const encryptionKey = deriveChildSeed(seed, inode);
    const metadata: IndependentFileSmallMetadata = {
    ...
    const revisionSeed = new Uint8Array(seed.length + 8);
    revisionSeed.set(seed, 0);
    const revisionKey = deriveChildSeed(revisionSeed, inode);
    const revision = BigInt(revisionKey[0]) * 256n + BigInt(revisionKey[1]);
    const [encryptedData, errEF] = encryptFileSmall(
      encryptionKey,
      inode,
      revision,
      metadata,
    ...
    const [immutableSkylink, errU] = await upload(STD_FILENAME, encryptedData);
    if (errU !== null) {
    ...
    const [entryData, errSTRED] = skylinkToResolverEntryData(immutableSkylink);
    if (errSTRED !== null) {
      resolve([{} as any, addContextToErr(errSTRED, "couldn't create resovler link from upload skylink")]);
      return;
    }
    const [, errRW] = await registryWrite(keypair, dataKey, entryData, revision);
    if (errRW !== null) {
    ...
    const skylink = entryIDToSkylink(entryID);

    // Create the view key, which is a composition of the inode and the
    // encryption key.
    const encStr = bufToB64(encryptionKey);
    const viewKey = encStr + inode;

    // Create and return the IndependentFile.
    const ifile: IndependentFileSmall = {
      dataKey,
      fileData,
      inode,
      keypair,
      ...
      readData: function (): Promise<[Uint8Array, Err]> {
        return new Promise((resolve) => {
          const data = new Uint8Array(ifile.fileData.length);
...
function openIndependentFileSmall(seed: Uint8Array, userInode: string): Promise<[IndependentFileSmall, Err]> {
  return new Promise(async (resolve) => {
    ...
    const [encryptedData, errD] = await download(skylink);
    if (errD !== null) {
    ...
    const encryptionKey = deriveChildSeed(seed, inode);
    const [metadata, fileData, errDF] = decryptFileSmall(encryptionKey, inode, encryptedData);
    if (errDF !== null) {
...
function viewIndependentFileSmall(skylink: string, viewKey: string): Promise<[IndependentFileSmallViewer, Err]> {
  return new Promise(async (resolve) => {
    ...
    const encStr = viewKey.slice(0, 22);
    const [encryptionKey, errBTB] = b64ToBuf(encStr);
    if (errBTB !== null) {
      resolve([{} as any, addContextToErr(errBTB, "unable to extract encryption key from view key")]);
      return;
    }
    const inode = viewKey.slice(22, viewKey.length);
    const [, fileData, errDF] = decryptFileSmall(encryptionKey, inode, encryptedData);
    if (errDF !== null) {
      resolve([{} as any, addContextToErr(errDF, "unable to decrypt file")]);
      return;
    }
    ...
    const ifile: IndependentFileSmallViewer = {
      fileData,
      skylink,
      viewKey,
...
function overwriteIndependentFileSmall(file: IndependentFileSmall, newData: Uint8Array): Promise<Err> {
  return new Promise(async (resolve) => {
    ...
    const newMetadata: IndependentFileSmallMetadata = {
    ...
    const [encodedRevision, errEU64] = encodeU64(file.revision);
    if (errEU64 !== null) {
      resolve(addContextToErr(errEU64, "unable to encode revision"));
      return;
    }
    const revisionSeed = new Uint8Array(file.seed.length + encodedRevision.length);
    revisionSeed.set(file.seed, 0);
    revisionSeed.set(encodedRevision, file.seed.length);
    const revisionKey = deriveChildSeed(revisionSeed, file.inode);
    const newRevision = file.revision + BigInt(revisionKey[0]) + 1n;

    // Get the encryption key.
    const encryptionKey = deriveChildSeed(file.seed, file.inode);
    ...
    // number.
    const [encryptedData, errEFS] = encryptFileSmall(
      encryptionKey,
      file.inode,
      newRevision,
      newMetadata,
    ...
    // Upload the data to get the immutable link.
    const [skylink, errU] = await upload(STD_FILENAME, encryptedData);
    if (errU !== null) {
    ...
    // Write to the registry entry.
    const [entryData, errSTRED] = skylinkToResolverEntryData(skylink);
    if (errSTRED !== null) {
      resolve(addContextToErr(errSTRED, "could not create resolver link from upload skylink"));
      return;
    }
    const [, errRW] = await registryWrite(file.keypair, file.dataKey, entryData, newRevision);
    if (errRW !== null) {
