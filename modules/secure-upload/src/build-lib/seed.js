...
var seedWordsToSeed = function (seedWords) {
		...
		if (seedWords.length !== dict.SEED_ENTROPY_WORDS) {
				return [null, new Error(`Seed words should have length ${dict.SEED_ENTROPY_WORDS} but has length ${seedWords.length}`)];
		}
		...
		for (let i = 0; i < dict.SEED_ENTROPY_WORDS; i++) {
				...
				for (let j = 0; j < dict.dictionary.length; j++) {
						if (seedWords[i].slice(0, dict.DICTIONARY_UNIQUE_PREFIX) === dict.dictionary[j].slice(0, dict.DICTIONARY_UNIQUE_PREFIX)) {
								word = j;
								break;
						}
				}
